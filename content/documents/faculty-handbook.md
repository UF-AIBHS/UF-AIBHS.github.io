# Faculty Handbook

**[Download the PDF ↓](../data/documents/AIBHS_Faculty_Handbook.pdf)**

AIBHS maintains two handbook sources — a narrative PDF (linked above) and a working
Word reference doc with links into UF systems. Both are archived, alongside the PDF
above, in `raw_projects_resources/` next to this repo (not published on the site).

## Sections and subsections

**From the narrative handbook (PDF):**

- Introduction to UF AIBHS
- Program Structure
- Onboarding
- Canvas
  - Adding TAs to Canvas
- Student Accommodations
  - Disability Resource Center
  - Providing Extra Time on Assignments
  - Deadline Extensions
- Student Engagement
  - Office Hours
  - Communication
- Semester Timeline
  - Pre-Semester
    - Requirements (Simple Syllabus sections)
    - The Process
    - Deadlines
  - Mid-Semester
  - End-Semester
    - Final Examinations
    - Last Week of Classes Policy
    - Submit Final Grades
    - Course Evaluations
- Important Dates and Policies
  - Academic Calendar
  - Add Drop
  - Religious Holidays
  - Reading Days

**From the working reference doc (Word):**

- How do I request a Canvas course?
- How do I set up my Canvas course?
  - How do I create Canvas assignments?
  - Creating Pages
  - Editing Pages
  - How do I change my course home page?
- How do I add TAs to Canvas?
- Student Accommodations
  - Disability Resource Center
  - Extra time
  - Deadline extensions
- Zoom Conferences
- Finalizing and Submitting Grades
- Importing from a Previous Canvas Course
- External Tools
  - Course Communication
  - EdStream
  - Automated Grading
  - Gradescope
  - Kaizen
  - Proctoring
  - Honorlock
  - Accessibility
  - Ally Accessibility Report
- Recommended Workflow
- College of Medicine Syllabus Template
- Office Hours Guidelines
- Important Dates and Policies
  - Academic Calendar
  - Add Drop
  - Religious Holidays
  - Reading Days
